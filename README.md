...
- 当前模型结论：过渡标签实验里 `v21` 仍最均衡，`v27` 质量监督回调实验已完成记录但仍未达到替换保留发布包的标准
...
| 模型实验 | `v27` 已完成记录，但 `v21` 仍是当前过渡标签实验里更均衡的一轮 |

当前不把实验轮次写成最终定版模型。现阶段的真实判断是：

- `v21` 样本级 `macro_f1 = 0.7185`，是当前过渡标签实验中更均衡的一轮
- `v22`、`v23`、`v24`、`v25`、`v26` 与 `v27` 都没有超过 `v21`
...
- 当前保留发布包仍不能被 `v21 / v22 / v23 / v24 / v25 / v26 / v27` 直接替换
...
- 模型线：优先验证过渡区间 `sample_weight` 进入 loss 层，而不是继续盲扫 `gamma / quality_loss_weight`
