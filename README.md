...
- 当前模型结论：`v29` 样本级整体分数超过 `v21`，但因 `near_fall` 回落仍进入人工复查口径，不直接冒充最终定版
- 最新负结果：`v30` 将 `sample_loss_weight` 提到 `0.65` 后，样本级 `macro_f1` 回落到 `0.6951`，不作为晋级方向
...
| 模型实验 | `v29` 样本级 `macro_f1 = 0.7355`，当前判读为 `review`，尚未直接替换保留发布包 |

当前不把实验轮次写成最终定版模型。现阶段的真实判断是：

- `v21` 样本级 `macro_f1 = 0.7185`，此前是过渡标签实验中更均衡的一轮
- `v22`、`v23`、`v24`、`v25`、`v26`、`v27` 与 `v28` 都没有超过 `v21`
...
- `v27` 把 `quality_loss_weight` 从 `0.10` 回调到 `0.08` 后，`fall / recovery / prolonged_lying` 再次抬升，但 `near_fall` 明显回落到 `0.5455`，样本级整体仍低于 `v21`
- `v28` 将 `sample_weight` 显式接入损失层后，`near_fall` 召回升到 `0.8000`，但精度降到 `0.2667`，样本级 `macro_f1 = 0.6675`，不晋级
- `v29` 将 `sample_loss_weight` 降到 `0.5` 后，样本级 `accuracy = 0.8902`、`macro_f1 = 0.7355`、`weighted_f1 = 0.8875`，整体超过 `v21`
- `v29` 的 `fall / recovery / prolonged_lying` 均优于 `v21`，但 `near_fall_f1` 从 `0.6667` 降到 `0.6000`
- `v30` 将 `sample_loss_weight` 提到 `0.65` 后，样本级 `macro_f1 = 0.6951`，`near_fall` 召回升到 `0.8000` 但精度降到 `0.4000`，不作为当前晋级方向
- 自动晋级判读为 `review`，需要进一步做应用级复核，不直接冒充最终定版
- 当前保留发布包仍不能被 `v21 / v22 / v23 / v24 / v25 / v26 / v27 / v28 / v29 / v30` 无条件替换
...
- 模型线：围绕 `v29` 做应用级复核，重点确认 `near_fall` 回落是否会影响实际提醒
