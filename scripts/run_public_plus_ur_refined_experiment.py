...
def build_refined_experiment_plan(
    *,
    python_bin: str,
    repo_root: Path,
    data_root: Path,
    ...
    if train:
        commands.append(
            {
                "step": "select_refined_deployment_checkpoint",
                "command": [
                    python_bin,
        ...
        commands.append(
            {
                "step": "evaluate_refined_sample_classification",
                "command": [
                    python_bin,
    ...
    if package_release:
        ...
        commands.append(
            {
                "step": "verify_refined_release_bundle",
                "command": [
                    python_bin,
                    "-m",
                    "huling_guard.cli",
    ...
    if baseline_run_name is not None:
        ...
        if baseline_batch_output_dir is not None and batch_output_dir is not None:
            commands.append(
                {
                    "step": "compare_refined_expected_state_vs_baseline",
                    "command": [
                        python_bin,
        ...
        baseline_train_summary = comparison_artifacts["previous_train_summary"]
        candidate_train_summary = run_dir / "summary.json"
        comparison_json = comparison_artifacts["comparison_json"]
        comparison_markdown = comparison_artifacts["comparison_markdown"]
        compare_command = [
            python_bin,
            "scripts/compare_experiment_results.py",
            "--baseline-train-summary",
            str(baseline_train_summary),
            "--candidate-train-summary",
            str(candidate_train_summary),
        ...
        commands.append({"step": "compare_refined_vs_baseline", "command": compare_command})
        commands.append(
            {
                "step": "assess_current_round_promotion",
                "command": [
                    python_bin,
...
def main() -> None:
    ...
    if args.resume:
        plan, skipped_steps = filter_plan_for_resume(
            plan,
            step_outputs=build_refined_experiment_step_outputs(
                data_root=data_root,
                run_name=args.run_name,
